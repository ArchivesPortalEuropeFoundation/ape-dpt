...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="metadata">
        <xsl:for-each-group select="./record" group-by="dc:identifier[1]">
            <!-- Provider aggregation -->
            <ore:Aggregation>
                <xsl:attribute name="rdf:about" select="concat('aggregation_', fn:replace(current-grouping-key(), ' ', '_'))"/>
                <edm:aggregatedCHO>
                    <xsl:attribute name="rdf:resource" select="concat('providedCHO_', fn:replace(current-grouping-key(), ' ', '_'))"/>
                </edm:aggregatedCHO>
                <edm:dataProvider>
                    <xsl:value-of select="current-group()[1]/europeana:dataProvider" />
                </edm:dataProvider>
                <xsl:for-each select="current-group()">
                    <xsl:if test='string-length(europeana:object) > 0'>
                        <edm:isShownBy>
                            <xsl:attribute name="rdf:resource" select="europeana:object" />
                        </edm:isShownBy>
                    </xsl:if>
                </xsl:for-each>
                <xsl:for-each select="current-group()">
                    <xsl:choose>
                        <xsl:when test='string-length(europeana:isShownAt) > 0'>
                            <edm:isShownAt>
                                <xsl:attribute name="rdf:resource" select="europeana:isShownAt"/>
                            </edm:isShownAt>
                        </xsl:when>
                        <xsl:otherwise>
                    ...
                    <edm:provider>
                        <xsl:value-of select="current-group()[1]/europeana:provider" />
                    </edm:provider>
                    <xsl:copy-of select="current-group()[1]/dc:rights" />
                    <edm:rights>
                        <xsl:attribute name="rdf:resource" select="current-group()[1]/europeana:rights" />
                    </edm:rights>
            </ore:Aggregation>
            ...
            <edm:ProvidedCHO>
                <xsl:attribute name="rdf:about">
                    <xsl:value-of select="concat('providedCHO_', fn:replace(current-grouping-key(), ' ', '_'))"/>
                </xsl:attribute>
                ...
                <xsl:if test='position() > 1'>
                    <dcterms:isPartOf>
                        <xsl:attribute name="rdf:resource" select="concat('providedCHO_', $repository_code, '_', $eadid)"/>
                    </dcterms:isPartOf>
                </xsl:if>
            ...
            <xsl:if test="$minimalConversion != 'true'">
                <xsl:if test='dcterms:alternative'>
                    <skos:Concept>
                        <xsl:attribute name="rdf:about">
                            <xsl:value-of select="concat('context_', fn:replace(current-grouping-key(), ' ', '_'))"/>
                        </xsl:attribute>
