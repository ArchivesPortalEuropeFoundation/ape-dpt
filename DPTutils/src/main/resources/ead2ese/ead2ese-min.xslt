...
http://purl.org/dc/terms/ http://www.dublincore.org/schemas/xmls/qdc/dcterms.xsd">
    ...
    <xsl:template name="addrecord">
        ...
        <xsl:if test="did/dao">
            ...
            <xsl:variable name="didnode" select="did"/>
	                    ...
	                    <xsl:when test="$useExistingDaoRole='true'">
	                        <xsl:choose>
	                            ...
	                            <xsl:when test=' "IMAGE" eq fn:string(@xlink:role)'>
	                           	 <xsl:attribute name="type"><xsl:text>IMAGE</xsl:text></xsl:attribute>
	                            </xsl:when>
	                            <xsl:when test=' "SOUND" eq fn:string(@xlink:role)'>
	                            <xsl:attribute name="type"><xsl:text>SOUND</xsl:text></xsl:attribute>
	                            </xsl:when>
	                            <xsl:when test=' "VIDEO" eq fn:string(@xlink:role)'>
	                            <xsl:attribute name="type"><xsl:text>VIDEO</xsl:text></xsl:attribute>
	                            </xsl:when>
	                            <xsl:when test=' "3D" eq fn:string(@xlink:role)'>
	                            <xsl:attribute name="type"><xsl:text>3D</xsl:text></xsl:attribute>
	                            </xsl:when>
	                            <xsl:otherwise>
	                            <xsl:attribute name="type"><xsl:value-of select="$europeana_type"/></xsl:attribute>
	                             </xsl:otherwise>
	                        </xsl:choose>
	                    </xsl:when>
	                    <xsl:otherwise>
