...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:europeana="http://www.europeana.eu/schemas/ese/" xmlns:dcterms="http://purl.org/dc/terms/"
	...
	<xsl:template match="/">
		<xsl:variable name="firstfiledirname"><xsl:call-template name="getDirName">
		...
		<xsl:variable name="firstfilename" select="concat($outputdir, '/', $firstfiledirname, '/1 ',fn:replace(/europeana:metadata/europeana:record[1]/dc:identifier, '[\\/\*\?:\|%@\[\] \n\t\r]', '_'),'.html')" />	
		<xsl:variable name="webfirstfilename" select="concat('htmlPreview?eseId=', $eseId,'&amp;path=',$outputdir, '/', $firstfiledirname, '/1%20',fn:replace(/europeana:metadata/europeana:record[1]/dc:identifier, '[\\/\*\?:\|%@\[\] \n\t\r]', '_'),'.html')" />				
		<xsl:variable name="weblistfilename" select="concat('htmlPreview?eseId=', $eseId,'&amp;path=', $outputdir, '/list.html')" />

		<html>
			<head>
				<title><xsl:value-of select="tokenize(document-uri(.), '/')[last()]"/></title>
		...
		<xsl:result-document href="{$listfilename}" method="html" encoding="UTF-8" version="4.0" omit-xml-declaration="yes" >
				<html>
					...
					<body>
						<h2><xsl:value-of select="count(/europeana:metadata/europeana:record) "/><xsl:text> records in </xsl:text><xsl:value-of select="tokenize(document-uri(.), '/')[last()]"/></h2>
						<xsl:if test="count(/europeana:metadata/europeana:record) > 0">
							<table cellspacing="1" cellpadding="0" width="100%" border="0" summary="search results" class="list">
								<th>Identifier</th>
								<th>Title</th>
							<xsl:for-each select="/europeana:metadata/europeana:record">
								<xsl:variable name="dirname"><xsl:call-template name="getDirName">
									<xsl:with-param name="currentRecordsPerDirectory" select="$startNumber"/>
									<xsl:with-param name="maxItemsPerDirectory" select="$maxItemsPerDirectory"/>
								...
								<td><a href="{$filename}" target="main"><xsl:value-of select="./dc:identifier" /></a></td><td> 
								<xsl:value-of select="./dc:title" /></td>
								</tr>
							</xsl:for-each>					
							</table>
						</xsl:if>					
		...
		<xsl:for-each select="/europeana:metadata/europeana:record">
								...
								<xsl:variable name="cssdirname"><xsl:call-template name="getCSSDirName">
									<xsl:with-param name="currentRecordsPerDirectory" select="$startNumber"/>
									<xsl:with-param name="maxItemsPerDirectory" select="$maxItemsPerDirectory"/>
									<xsl:with-param name="position" select="position()"/>
					...
					<body>
						<table  id="multi"  cellspacing="10" cellpadding="10"> 
							<tr>
								<td valign="top">
									...
									<xsl:choose>
										<xsl:when test="./europeana:object">
											<xsl:variable name="href" select="./europeana:object"/>	
											<img width="200px" src="{$href}"></img>
										</xsl:when>
										<xsl:otherwise>
											<xsl:variable name="type" select="fn:lower-case(./europeana:type)"/>	
								...
								<td>					
						<div id="item-detail">
							<h2 class="{./europeana:type}">
								<xsl:value-of select="./dc:title" />
							</h2>
							<xsl:for-each select="child::*">
								<p><strong><xsl:value-of select="./name()" />:<xsl:text> </xsl:text></strong>
									<xsl:choose>
...
<xsl:template name="getDirName">
	...
	<xsl:variable name="temp" select="floor($position div $currentRecordsPerDirectory) * $currentRecordsPerDirectory"></xsl:variable>
	
	<xsl:choose>
		<xsl:when test="$currentRecordsPerDirectory > $maxItemsPerDirectory">
			<xsl:call-template name="getDirName">
				<xsl:with-param name="position" select="$position"/>
				<xsl:with-param name="currentRecordsPerDirectory" select="$currentRecordsPerDirectory div $maxItemsPerDirectory"/>
				<xsl:with-param name="maxItemsPerDirectory" select="$maxItemsPerDirectory"/>
